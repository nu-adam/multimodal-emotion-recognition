...
class VideoEncoder(nn.Module):
    """
    Combines VGG-16 and Transformer Encoder for video modality.

    Methods:
        forward(x): Extracts and encodes video features.
    """
    ...
    def forward(self, x):
        """
        Forward pass through the video feature extractor.

        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, frames, C, H, W).

        Returns:
            torch.Tensor: Encoded video features of shape (batch_size, embed_dim).
        """
